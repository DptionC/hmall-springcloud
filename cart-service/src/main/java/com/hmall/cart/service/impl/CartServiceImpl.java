...
public class CartServiceImpl extends ServiceImpl<CartMapper, Cart> implements ICartService {
    ...
    //已经添加了@RequiredArgsConstructor，并加上了final修饰符，这个类就说明必须进行初始化
    ...
    private void handleCartItems(List<CartVO> vos) {
        ...
        //2.3 利用restTemplate向商品管理发起http请求,得到http的响应
        ResponseEntity<List<ItemDTO>> response = restTemplate.exchange(
                uri + "/items?ids={ids}",
                HttpMethod.GET,
                null,
                new ParameterizedTypeReference<List<ItemDTO>>() {
                }, //需要创建一个参数化类型引用对象,写入自己需要的类型,再利用反射拿到这个对象的泛型,从而获取需要的类型
                Map.of("ids", CollUtil.join(itemIds, ",")) //后者作用,把集合告诉我,用逗号进行拼接的意思
        );
        //2.2 解析响应,获取响应体信息
